...
bool check(const string_view& str, int i) {
    switch (i) {
        ...
        case 5: {
            if (str.size() != 5 || str[0] != 'g' || str[1] != '.' || !isupper(str[2]) || !isupper(str[3]) || !isupper(str[4])) {
                return false;
            }
            return true;
        }
        ...
        case 6: {
            if (str.size() < 5 || str[0] != 'u' || str[1] != 'l' || !isupper(str[2])) {
                return false;
            }

            for (int j = 3; j < str.size() - 1; j++) {
                if (!islower(str[j])) {
                    return false;
                }
            }
